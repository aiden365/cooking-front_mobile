...
import { Category, Horizontal, Left, More } from '@nutui/icons-vue'
...
type IndividualDishContentSnapshot = {
  dishName?: string
  takeTimes?: string
  take_times?: string
  materials?: IndividualDishMaterial[]
  flavors?: IndividualDishFlavor[]
  steps?: IndividualDishStep[]
  tips?: string
}
...
function applySnapshot(snapshot: IndividualDishContentSnapshot) {
  baseInfo.value = {
    dishName: snapshot.dishName || '',
    take_times: snapshot.take_times,
    takeTimes: snapshot.takeTimes ?? snapshot.take_times,
  }
  materials.value = snapshot.materials || []
  flavors.value = snapshot.flavors || []
  steps.value = [...(snapshot.steps || [])].sort(
    (left, right) => left.stepNumber - right.stepNumber,
  )
  tips.value = snapshot.tips || ''
}

async function loadDetail() {
  if (!individualDishId.value) {
    loading.value = false
    errorMessage.value = '缺少个性化菜谱信息'
    return
  }
  ...
  try {
    ...
    if (!response.data.content.trim()) {
      throw new Error('个性化菜谱内容为空')
    }

    const snapshot = JSON.parse(response.data.content) as IndividualDishContentSnapshot
...
function goBack() {
  ...
  router.push('/user/my-individual-dish')
}
    ...
    <main class="page-content">
      <section class="status-card" :class="{ 'status-card-error': errorMessage }">
        <div class="status-icon">
          <Category size="16" :color="errorMessage ? '#ff5a5a' : '#ff9f43'" />
        </div>
