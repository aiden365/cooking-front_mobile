...
> **开发者指令：**
> 现在，让我们开始`菜谱搜索功能优化`吧
> 0. 请在页面 [dish-search.vue](src/views/dish/dish-search.vue) 及dish.ts等相关文件中完成功能
> 1. 目前的搜索是直接通过调用/dish/page接口然后让我们改进一下，这个搜索流程。我希望它首先调用菜谱接口，这个接口会根据向量检索返回最相似的菜品id列表，然后根据菜品id列表去调用/dish/page接口，然后返回给前端。
> 2. 菜谱搜索接口 post  /dish/search 请求参数 {dishName}
> 3. 如果菜谱搜索接口返回的菜谱id列表为空，则表示没有检索到相关的菜谱。
> 4. 
